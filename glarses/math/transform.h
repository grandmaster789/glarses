...
namespace glarses::math {
	...
	public:
		...
		[[nodiscard]] const glm::quat& get_orientation() const;

		Transform&                     set_scale(float x, float y, float z);
		Transform&                     set_scale(const glm::vec3& v);
		[[nodiscard]] const glm::vec3& get_scale() const;

        [[nodiscard]] glm::vec3 to_local_frame(const glm::vec3& point);
        [[nodiscard]] glm::vec3 to_parent_frame(const glm::vec3& point);

		[[nodiscard]] glm::mat4 get_matrix_to_local() const;
        [[nodiscard]] glm::mat4 get_matrix_to_parent() const;
